package com.deeplake.workbenchfc.item.skills;

import com.deeplake.workbenchfc.IdlFramework;
import com.deeplake.workbenchfc.init.ModCreativeTab;
import com.deeplake.workbenchfc.item.ItemBase;
import com.deeplake.workbenchfc.util.CommonFunctions;
import com.deeplake.workbenchfc.util.IDLSkillNBT;
...
import static com.deeplake.workbenchfc.util.CommonDef.G_SKY;
import static com.deeplake.workbenchfc.util.CommonDef.TICK_PER_SECOND;
import static com.deeplake.workbenchfc.util.CommonFunctions.SendMsgToPlayer;
import static com.deeplake.workbenchfc.util.CommonFunctions.isShiftPressed;
import static com.deeplake.workbenchfc.util.MessageDef.getSkillCastKey;
...
public class ItemSkillBase extends ItemBase {
    ...
    {
        CommonFunctions.SendMsgToPlayer(player, "workbenchfc.skill.msg.cool_down");
    }
    ...
    public void addInformation(ItemStack stack, World world, List<String> tooltip, ITooltipFlag flag) {
        ...
        {
            if (showCDDesc)
                tooltip.add(I18n.format("workbenchfc.skill.shared.cool_down_desc", getCoolDown(stack)));
            if (showDamageDesc && getVal(stack) > 0)
                tooltip.add(I18n.format("workbenchfc.skill.shared.power_desc", getVal(stack)));
            if (showRangeDesc)
                tooltip.add(I18n.format("workbenchfc.skill.shared.range_desc", getRange(stack)));
            if (showDuraDesc)
                tooltip.add(GetDuraDescString(getDura(stack)));

            if (maxLevel != 1)
            {
                tooltip.add(I18n.format("workbenchfc.skill.shared.level_desc", IDLSkillNBT.getLevel(stack), maxLevel));
            }
        }
    }
...
//    	}
        ...
        String strMaxLv = lv == maxLevel ? I18n.format("workbenchfc.skill.shared.lv_max") : "";

        return I18n.format("workbenchfc.skill.shared.name_format_with_lv",strMain, IDLSkillNBT.getLevel(stack), strMaxLv);
    }

    public String GetDuraDescString(float val)
    {
        return I18n.format("workbenchfc.skill.shared.duration_desc", val);
    }
